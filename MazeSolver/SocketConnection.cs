...
namespace MazeSolver
{
    public class SocketConnetion
    {
        ...
        public string ReceiveUntil(string i_StringToLookFor, string i_AnotherOption = "")
        {
            string result = "";

            try
            {
                if (string.IsNullOrEmpty(i_AnotherOption))
                {
                ...
                {
                    while (!result.Contains(i_StringToLookFor) && !result.Contains(i_AnotherOption))
                    {
                        // Receive the response from the remote device.  
                        int bytesRec = this.m_Socket.Receive(this.m_Buffer);
                        result += Encoding.ASCII.GetString(this.m_Buffer, 0, bytesRec);
